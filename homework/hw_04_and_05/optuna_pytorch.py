...
class NeuralNetwork(torch_nn.Module):
    def __init__(
        ...
        train_U, self.train_S, train_V = pca_lowrank(
            from_numpy(self.np_train_data).double(),
            niter=self.pca
        )
        test_U, self.test_S, test_V = pca_lowrank(
    ...
    def make_hidden_layers(self):
        self.fc1 = torch_nn.Linear(10 * 20, self.hidden_layer_dimensions)  # FIXME
    ...
    def forward(self, input):
        flatten_image_input = input.view(-1, 20 * 10).to(self.device)  # FIXME
